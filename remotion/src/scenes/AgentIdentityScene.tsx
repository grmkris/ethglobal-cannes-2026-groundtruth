...
export const AgentIdentityScene: React.FC = () => {
  ...
  // Phase 1: Agent identity art (120-320)
  const img1Opacity = interpolate(frame, [110, 150], [0, 1], { extrapolateRight: "clamp", extrapolateLeft: "clamp" });
  const img1Fade = interpolate(frame, [280, 330], [1, 0], { extrapolateRight: "clamp", extrapolateLeft: "clamp" });
  const img1Scale = interpolate(frame, [120, 330], [1, 1.06], { extrapolateRight: "clamp", extrapolateLeft: "clamp" });

  // Phase 2: ENS registration (300-650)
  const img2Opacity = interpolate(frame, [290, 330], [0, 1], { extrapolateRight: "clamp", extrapolateLeft: "clamp" });
  const img2Fade = interpolate(frame, [610, 660], [1, 0], { extrapolateRight: "clamp", extrapolateLeft: "clamp" });
  const img2Scale = interpolate(frame, [300, 660], [1, 1.05], { extrapolateRight: "clamp", extrapolateLeft: "clamp" });

  // Phase 3: Agent view (630-900)
  const img3Opacity = interpolate(frame, [620, 660], [0, 1], { extrapolateRight: "clamp", extrapolateLeft: "clamp" });
  const img3Fade = interpolate(frame, [860, 910], [1, 0], { extrapolateRight: "clamp", extrapolateLeft: "clamp" });
  const img3Scale = interpolate(frame, [630, 910], [1, 1.05], { extrapolateRight: "clamp", extrapolateLeft: "clamp" });

  // Phase 3.5: Statistics (880-1100)
  const img4Opacity = interpolate(frame, [870, 910], [0, 1], { extrapolateRight: "clamp", extrapolateLeft: "clamp" });
  const img4Fade = interpolate(frame, [1060, 1100], [1, 0], { extrapolateRight: "clamp", extrapolateLeft: "clamp" });
  const img4Scale = interpolate(frame, [880, 1100], [1, 1.04], { extrapolateRight: "clamp", extrapolateLeft: "clamp" });

  // Phase 4: Text (1090-end)
  const textBgOpacity = interpolate(frame, [1080, 1110], [0, 1], { extrapolateRight: "clamp", extrapolateLeft: "clamp" });
  const textScale = spring({ frame: Math.max(0, frame - 1100), fps, config: SPRING.default });
  const textOpacity = interpolate(frame, [1090, 1120], [0, 1], { extrapolateRight: "clamp", extrapolateLeft: "clamp" });

  // Labels
  const label2Opacity = interpolate(frame, [340, 370], [0, 1], { extrapolateRight: "clamp", extrapolateLeft: "clamp" });
  const label3Opacity = interpolate(frame, [670, 700], [0, 1], { extrapolateRight: "clamp", extrapolateLeft: "clamp" });
  const label4Opacity = interpolate(frame, [920, 950], [0, 1], { extrapolateRight: "clamp", extrapolateLeft: "clamp" });
